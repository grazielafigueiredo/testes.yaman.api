Dado("que adiciono um max regular no carrinho e um pré venda") do
    ...
    # all('div[class*="card-vitrine__pacote"]')[5].click #escolher dezenas
    ...
    find('div.escolhaModal__body.escolhaModal__body--escolha > ul > li:nth-child(08)').click
...
Quando("vou confirmar se as opcões {string} estao bloqueadas") do |forma_pagamento_desabilitada|
    pagamentos_bloqueados = all('li[class*="tab-list-item tab-list-item--disabled"]')
