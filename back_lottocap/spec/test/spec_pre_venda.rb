...
context 'Validar se há dezenas repetidas' do
  before do
    ...
    conjuntos = JSON.parse(@show_dezenas.response.body)['obj']
    puts @show_dezenas
  ...
  it 'exibirDezenas' do
    expect(JSON.parse(@show_dezenas.response.body)['obj'][0]['idDezenas']).to be_a Integer
    expect(JSON.parse(@show_dezenas.response.body)['obj'][0]['dezenas']).to be_a String
    expect(JSON.parse(@show_dezenas.response.body)['sucesso']).to be true
  end
end
...
context 'Quando a série não estiver em pre venda, então o contrato do endpoint não pode exibir conjunto de dezenas' do
  ...
  it 'Quando a série não estiver em pre venda, então o contrato do endpoint não pode exibir conjunto de dezenas' do
    expect(JSON.parse(@show_dezenas.response.body)['obj']).to eql []
  end
end
...
context 'Concorrencia no pagamento Título já reservado' do
  before do
    ...
    @group_dezenas = JSON.parse(@search_dezenas.response.body)['obj'][0]

    @cart = build(:cart_dezenas).to_hash
    @cart[:lstDezenas] = [@group_dezenas]
    @cart_pre = ApiPreVenda.post_add_item_cart_prevenda(@token, @cart)
    @id_cart = JSON.parse(@cart_pre.response.body)['obj'][0]['idCarrinho']
  ...
  it 'Pagar conjunto de dezenas que já foi reservado/comprado' do
    expect(JSON.parse(@result.response.body)['sucesso']).to eql false
    expect(JSON.parse(@result.response.body)['erros'][0]['mensagem']).to eql "Reserva de titulos já foi solicitada para as dezenas #{@group_dezenas}"
  end
